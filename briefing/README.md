...
> **체크 상태 되읽기 (2026-08-12 정정)**: **봇 Web API**로는 캔버스 체크 상태를 못 읽는다
> (`canvases.read` 부재). 하지만 **claude.ai Slack 커넥터의 `slack_read_canvas`**는 캔버스를
> 마크다운으로 돌려주며 체크리스트를 `- [x]`(체크)/`- [ ]`(미체크)로 그대로 반환한다(실측 확인).
> 브리핑은 커넥터로 돌므로(ALLOWED에 `slack_read_canvas` 추가 시) **어제 캔버스의 완료여부를
> 읽어 이월·집계·전사현황 소스로 쓸 수 있다.** `canvas-sync.py`(봇)는 생성/프루닝만 하고,
> 되읽기는 브리핑 모델(커넥터)이 담당하는 분업. (되읽기 배선은 필요 시 켠다.)
